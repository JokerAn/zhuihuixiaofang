...
export class MyControlComponent implements OnInit {
  ...
  openAll() { 
    ...
    this.anHttp.get('https://uatapi.iqcspace.com/api-client/front/equipment/v2/actuator/' + 'Cabinet-text-all/501?waitForResponse=false&a='+suijishu, {})
      .subscribe((result: any) => {
        console.log(result);
        this.anData.alertMsgShowF(result.msg);
      })
  ...
  openKaiGui(res: any) { 
    let suijishu = +new Date();
    this.anHttp.get('https://uatapi.iqcspace.com/api-client/front/equipment/v2/actuator/' + res.id + '/501?waitForResponse=false&a=' + suijishu, {})
      .subscribe((result: any) => {
        console.log(result);
        this.anData.alertMsgShowF(result.msg);
      })
